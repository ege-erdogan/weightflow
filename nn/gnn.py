...
class GNNForClassification(nn.Module):
    def __init__(
        ...
        jit=False,
        refine=False,
        task_level: str = "edge_node",  # 'graph', 'node', 'edge'
    ):
        super().__init__()
        self.refine = refine
        self.device = device
        ...
        self.construct_graph = graph_constructor
        ...
        if backbone == "PNA":
            self.gnn = PNA(
                ...
                modulate_edges=True,
                gating_edges=False,
                final_edge_update=True,
            )
        else:
            raise Exception("GNN backbone not supported")
    ...
    def _incorporate_time(self, inputs, t):
        # here t is a tensor (n_batch, dim) each elem contains the same t
        # if t is not None:
        #     t = Batch.deflatten(
        #         t,
        ...
        if t is not None:
            edge_features = edge_features.reshape(-1, self.d_hid - 1)
            edge_features = torch.cat(
                [
                    edge_features,
                    # torch.ones(edge_features.size(0), 1, device=self.device),
                    torch.repeat_interleave(
                        times, int(edge_features.size(0) / times.size(0))
                    ).unsqueeze(dim=-1),
                ],
                dim=-1,
            ).reshape(-1, n_nodes, n_nodes, self.d_hid)

            # edge_features[:, self.edge_index[0], self.edge_index[1], -1] = t.flatten()[
            #     :n_edges
            # ].to(self.device)

            node_features = node_features.reshape(-1, self.d_hid - 1)
            node_features = torch.cat(
                [
                    node_features,
                    # torch.ones((node_features.size(0), 1), device=self.device),
                    torch.repeat_interleave(
                        times, int(node_features.size(0) / times.size(0))
                    ).unsqueeze(dim=-1),
                ],
                dim=-1,
            )
            # input_node_mask = torch.zeros((self.n_batch, n_nodes), dtype=torch.int)
            # input_node_mask[:, n_input_nodes:] = 1
            # bias_mask = input_node_mask.bool().flatten()
            # node_features[bias_mask, -1] = t.flatten()[-bias_mask.sum() :].to(
            #     self.device
            # )

            edge_features = edge_features.view(self.n_batch, n_nodes, n_nodes, -1)
            node_features = node_features.view(self.n_batch, n_nodes, -1)
    ...
    def forward(self, inputs: torch.Tensor, t: torch.Tensor | None = None):
        ...
        # inputs is of shape (n_batch, wb_flat_dim)
        # t is of shape (n_batch)
        ...
        node_features = out_node.reshape(batch.num_graphs, n_nodes, out_node.shape[-1])
        edge_features = to_dense_adj(batch.edge_index, batch.batch, out_edge)

        if self.task_level == "graph":
            graph_features = self.pool(node_features, edge_features)
            return self.proj_out(graph_features).squeeze()
        elif self.task_level == "edge_node":
...
class PNA(BasicGNN):
    ...
    def init_conv(
        ...
        return PNAConv(in_channels, out_channels, edge_dim=in_channels, **kwargs)
